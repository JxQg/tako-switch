...
- 顶层 `default_permissions`：`:read-only`、`:workspace`、`:danger-full-access`。
- `[windows].sandbox`：`elevated`、`unelevated`。
- `[features]`：`js_repl`、`unified_exec`、`shell_snapshot`、`memories`。

“开启完全访问权限”只是前端快捷项：如果用户正在使用 `default_permissions`，写入 `default_permissions = ":danger-full-access"`；否则写入 `sandbox_mode = "danger-full-access"` 和 `approval_policy = "never"`。不要把 `[windows].sandbox = "elevated"` 当作完全访问权限。macOS 系统层面的 Full Disk Access、Accessibility、Screen Recording 只能作为说明，不应由 Tako Switch 自动写入或修改。
...
导入高级配置通过 `ConfigInput.platforms.claude.options` 传入。`permissions.defaultMode` 是枚举值，当前允许 `default`、`acceptEdits`、`plan`、`auto`、`dontAsk`、`bypassPermissions`；`skipDangerousModePermissionPrompt` 是布尔值。未传入时必须保留用户原来的 `permissions` 和顶层危险提示字段。`skipDangerousModePermissionPrompt` 只有在 `permissions.defaultMode = "bypassPermissions"` 的 UI 路径下可选，避免用户把高风险组合当成普通加速项。
